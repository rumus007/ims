        ...
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="main-card mb-3 card">
                    <div class="card-body">
                        <form id="my-form" class="" method="post" action="{{route('roles.store')}}">
                            {{csrf_field()}}
                            <div class="position-relative row form-group"><label for="roleName" class="col-sm-2 col-form-label">Role Name</label>
                                <div class="col-sm-10">
                                    <input name="name" id="roleName" placeholder="Enter the role name" type="text" class="form-control" required>
                                </div>
                            </div>
                            <div class="position-relative row form-group"><label for="displayName" class="col-sm-2 col-form-label">Display Name</label>
                                <div class="col-sm-10">
                                    <input name="display_name" id="displayName" placeholder="Enter the display name here" type="text" class="form-control" required>
                                </div>
                            </div>
                            <div class="position-relative row form-group"><label for="description" class="col-sm-2 col-form-label">Description</label>
                                <div class="col-sm-10">
                                    <input name="description" id="description" placeholder="Enter description of role here here" type="text" class="form-control" required>
                                </div>
                            </div>
                            <div class="col-lg-12">
                                <div class="main-card mb-3 card">
                                    <div class="card-body"><h5 class="card-title">Permissions</h5>
                                        <table class="mb-0 table table-sm">
                                            ...
                                            <tr>
                                                <th scope="row">{{$i + 1}}</th>
                                                @for($j = 0; $j <=4; $j++)
                                                    <td><input type="checkbox" name="roles[]" id="checkbox-{{$permissions[5 * $i + $j]['id']}}" value="{{$permissions[5 * $i + $j]['id']}}"><label for="checkbox-{{$permissions[5 * $i + $j]['id']}}">{{$permissions[5 * $i + $j]['display_name']}}</label></td>
                                                @endfor
                                            </tr>
                                            @endfor
                                            </tbody>
